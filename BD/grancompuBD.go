...
func Monitores(item grancompu.Item) {
	...
	monitor.CodigoProducto = item.Producto["código producto"]
	...
	monitor.Monitorescol = item.Producto["monitores col"]
	...
	stmt, es := db.Prepare("INSERT INTO Monitores (Fecha, OC, Suc, Familia, CodigoProducto, Serie, SerieOriginal, Marca, Modelo, Forma, " +
		" Base, Monitorescol, Tipo, Salidas, HDMI, Clase, Tamaño) " +
	...
	a, err := stmt.Exec(monitor.Fecha, monitor.OC, monitor.Suc, monitor.Familia, monitor.CodigoProducto, monitor.Serie, monitor.SerieOriginal,
		monitor.Marca, monitor.Modelo, monitor.Forma, monitor.Base, monitor.Monitorescol, monitor.Tipo, monitor.Salidas, monitor.HDMI,
...
func GetMonitores() (Data []modelos.Monitor) {
	listado, _ := db.Query("SELECT Fecha, OC, Suc, Familia, CodigoProducto, Serie, SerieOriginal, Marca, Modelo, Forma," +
		" Base, Monitorescol, Tipo, Salidas, HDMI, Clase, Tamaño FROM Monitores WHERE Activo=1;")
	revisarError(err)
	for listado.Next() {
		err = listado.Scan(
			&Fecha,
...
func GetMonitoresInactivo() (Data []modelos.Monitor) {
	listado, _ := db.Query("SELECT Fecha, OC, Suc, Familia, CodigoProducto, Serie, SerieOriginal, Marca, Modelo, Forma," +
		" Base, Monitorescol, Tipo, Salidas, HDMI, Clase, Tamaño, FechaVent, SerieDoc, DocVent  FROM Monitores WHERE Activo=0;")
	revisarError(err)
	for listado.Next() {
		err = listado.Scan(
			&Fecha,
...
func ActualizaMonitor(Monitor modelos.Monitor) (resp modelos.RespuestaSencilla) {
	...
	stmt, es := db.Prepare("UPDATE Monitores SET Fecha=?, OC=?, Suc=?, Familia=?, CodigoProducto=?, Serie=?, SerieOriginal=?, Marca=?," +
		" Modelo=?, Forma=?,Base=?, Monitorescol=?, Tipo=?, Salidas=?, HDMI=?, Clase=?, Tamaño WHERE IdMonitores=?;")
	if es != nil {
		panic(es.Error())
	}
	a, err := stmt.Exec(Monitor.Fecha, Monitor.OC, Monitor.Suc, Monitor.Familia, Monitor.CodigoProducto, Monitor.Serie, Monitor.SerieOriginal,
